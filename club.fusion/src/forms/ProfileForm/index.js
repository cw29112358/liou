...
class ProfileForm extends React.Component {
  ...
  renderVerifiedButton = (isPending) => {
    // isPending ？灰色按钮 ：蓝色渐变按钮
    const linearProps = isPending ? {} : {
      ...styles.linearProps,
      shadowStyle: [styles.brandShadow, styles.linearShadow],
    };
    const buttonStyle = [styles.linearButton];
    if (isPending) buttonStyle.push(styles.greyVerifyButton);

    return (
      <View style={styles.verifyView}>
        <Button
          disabled={isPending}
          onPress={this.onPressVerified}
          style={buttonStyle}
          textLabel={`verify${isPending ? 'Pending' : 'To'}`}
  ...
  render() {
    const { isOthers, areaOptions, authUser: { verificationStatus, verifiedFund } } = this.props;
    const { showChunk } = this.state;
    ...
    let fields = [
      ...
      },
      {
        ...
        hideOptions: true,
        children: unVerified ? this.renderVerifiedButton(isPending) : undefined,
      },
    ];
