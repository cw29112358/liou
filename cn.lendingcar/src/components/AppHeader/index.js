...
const AppHeader = (props) => {
  const {
    ...
    headerSettings, isTranslate, hasShadow, hiddenBorder, headerStyle,
  ...
  const renderLeftIcon = (image, iconName, style) => {
    ...
    if (iconName) {
      ...
      if (transparent) iconStyle.push(styles.colorWhite);
  ...
  const fields = [
    ...
    },
    {
      ...
      childrenProps: {
        textLabel: title,
        textStyle: [styles.title, transparent ? styles.colorWhite : undefined, titleStyle],
        disabled: true,
  ...
  const newHeaderStyle = [styles.headerStyle];
  if (!transparent && hasShadow) newHeaderStyle.push(styles.shadow);
  if (!transparent && hiddenBorder) newHeaderStyle.push(styles.noBorderBottom);
  if (headerStyle) newHeaderStyle.push(headerStyle);

  const header = (
    <Header {...otherProps} style={newHeaderStyle}>
