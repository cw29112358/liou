...
class AgentEcharts extends React.Component { // eslint-disable-line
  renderEcharts() {
    ...
    const translatedOneMonth = (<TabHeading><Text style={styles.tabTextStyle}> { translate('oneMonth') } </Text></TabHeading>);
    const translatedThreeMonth = (<TabHeading><Text style={styles.tabTextStyle}> {translate('threeMonth')} </Text></TabHeading>);
    return (
      <View style={styles.echarts}>
        ...
        {<Echarts option={option} height={getPadSize(182, 2.5)} />}
      </View>
    );
  }
