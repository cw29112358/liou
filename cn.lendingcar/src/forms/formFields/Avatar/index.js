...
class Avatar extends React.Component {
  ...
  renderPhotoUpload() {
    const {
      fileType, input, avatarProps, avatarStyle,
      isShowUserName, authUser, nameStyle,
    } = this.props;
    return (
      <PhotoUpload
      ...
      >
        <MyAvatar
          ...
          userName={isShowUserName ? getUserDisplayName(authUser) : undefined}
          nameStyle={[styles.nameStyle, nameStyle]}
  ...
  renderMembershipDate(authUserMembership) {
    ...
    const memberDate = `${momentFormat(startDate)} ${AVATAR_DATE_SEPARATOR} ${momentFormat(endDate)}`;
