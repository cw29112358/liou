...
export function getIndustries() {
  return getStrapi('industry.json');
}
...
export function getActivitiesApi(params = {}) {
  ...
  const appIdParams = Object.assign(urlParams, { appId: 'gmu' });
  return getStrapi(url, {
    params: appIdParams, // { area, industry, user }
  });
}
...
export function stepUpViewCount(id) {
  const url = `api/activities/stepUpViewCount/${id}`;
  return getStrapi(url);
}
...
export function getMembershipPrice() {
  const url = 'api/memberships/price';
  return getStrapi(url);
}
