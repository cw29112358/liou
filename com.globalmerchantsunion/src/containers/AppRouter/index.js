...
class AppRouter extends React.Component {
  ...
  initPush = () => {
    if (!isIOS) {
      XGPush.init(2100326681, 'AS425K2GJ4XZ');// ACCESS ID+ACCESS KEY
    } else {
      XGPush.init(2200326682, 'INJ2581UXH9D');
    }
    this.initXGRegister();
  }
  // 注册
  initXGRegister = () => {
    const packageName = !isIOS ? 'com.globalmerchantsunion' : 'com.gmu';
  ...
  async loadHomeSceneData() {
    const {
      ...
      loadLocationCity, loadAreaConfig, loadIndustries, loadActivities,
    } = this.props;
...
AppRouter.propTypes = {
  ...
  loadIndustries: PropTypes.func.isRequired,
...
const mapDispatchToProps = (dispatch) => ({
  ...
  loadIndustries: () => dispatch(loadIndustriesAction()),
