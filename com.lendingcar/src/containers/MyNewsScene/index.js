...
import {
  loadBenefitsAction,
} from 'containers/MemberBenefitScene/actions';
import {
  withReducer as benefitReducer,
  withSagas as benefitSagas,
} from 'containers/MemberBenefitScene';
...
export class MyNewsScene extends React.Component {
  ...
  componentWillMount() {
    const { notificationId, loadBenefits } = this.props;
    loadBenefits();
      ...
      <View style={styles.contentLeft}>
        ...
        {(item.carId || item.type === 'car') && (
          <View style={styles.tagLine}>
            {this.renderTag(item.metadata.depositPrice, 'itemDeposit')}
            {/* {this.renderTag(item.metadata.depositFinance.monthlyPayment, 'monthly')} */}
          </View>
        )}
      </View>
    );
  ...
  renderContent = () => {
    ...
    return (
      ...
      >
        ...
        { !isLoading && <SeperatorText label="noNews" showSeperate="true" />}
      </ScrollView>
    );
  }
  render() {
