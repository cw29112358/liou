...
export function getCalculatedString(make) {
  return (make && make.trim())
    ? _.camelCase(make).match(/[a-z]+/ig).join('')
    : '';
}
...
export const notificationLink = (promotionUrl) => {
  if (promotionUrl.indexOf('http') !== -1) {
    ...
    Actions.push(routeParams[0] || 'member', { messageId: routeParams[1] });
  }
};

export const getCurrentDate = () => moment().format('YYYY-MM-DD h:mm:ss a');
...
export const getUserPhoneNumberArray = (userPhoneNumber) => {
  ...
  if (userPhoneNumberArray.length < 2) {
    // old data just only phoneNumber for US
    return [1, userPhoneNumberArray[0]];
  }
  return userPhoneNumberArray;
};
export const formatSubmitPhoneNumber = (countryCode, phoneNumber) => `${countryCode}-${phoneNumber}`;
